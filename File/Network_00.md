...
从上文的DNS查询过程来看，DNS的查询过程需要递归（或者迭代）遍历多个DNS服务器，才能获得最终的解析结果，这会带来一定的时延，甚至会解析超时。
...
## 怎么办！
面对传统DNS服务的种种困境，产生了一种新解决方案----HttpDNS。直接http访问HttpDNS供应商的DNS服务器，最快的获取IP地址。这个之后有机会再讲。

# 总结
DNS是解决域名--IP地址转换的方案，它在最开始的时候只使用UDP作为传输协议。但是随着时间的推移，网络环境越来越复杂，DNS也在不断的更新和优化，不断地将TCP、TLS、HTTPS加入进来。

而DNS可以说是网络的基石之一，但是也并非完美，HttpDNS就是一种解决方案。
