...
我们但凡开始做 iOS 开发的那一天开始，就经常听到关于 UI 不能在子线程操作的警告。我们已经把这句话都快融进骨子里了，但是这种事情还是会时有发生，最典型是错误就是在子线程里去进行图片的加载。
...
#endif
```
strcmp()是c语言的字符串比较函数. strcmp(s1，s2) 判断两个字符串s1和s2是否相同，相同 == 0； 如果当前队列已经是主队列了，那么直接运行下去；如果当前队列不是主队列，调用`dispatch_async(dispatch_get_main_queue(), block)`。 通过这样的判断，可以保证是是主队列。
...
> 2. hook 所有的 UIView 的`setNeedsLayout`、`setNeedsDisplay`、`setNeedsDisplayInRect:`的方法，在线上做好防护，让所有子线程的UI操作强制回到主线程上去。
